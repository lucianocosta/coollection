...
It doesn't matter anymore to Coollection if your filter is a method nor a public or private field. If it's actually there, you'll get it!

<pre>
// will try to match first for "public void name();"
// and than to "private Object name;"
from(animals).where("name", eq("Cat")).all();

// this also will work
from(animals).where("getName", eq("Cat")).all();
</pre>

Was added the matcher *eqi()*, wich states for *equals ignore-case*, or *equals insensitive*.

<pre>
from(animals).where("name", eqi("cAt")).all();
</pre>
